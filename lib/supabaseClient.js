...
export async function adminRewriteScheduleStartNow(supabase, id) {
  ...
  if (schedule.hard_end) {
    duration = differenceInSeconds(new Date(schedule.hard_end), new Date(schedule.start));
    newHardEnd = add(now, { seconds: duration });
  }

  console.log("moving start + end + hard_end to now: ",
    toDB(newStart),
    toDB(newEnd),
    toDB(newHardEnd),
    "from",
  ...
  let { error: updateScheduleError } = await supabase
    .from('schedules')
    .update({ start: toDB(newStart), end: toDB(newEnd), hard_end: toDB(newHardEnd) })
    .eq('id', id);
  ...
  for (let item of items) {
    ...
    let { error: updateItemError } = await supabase
      .from('items')
      .update({ start_at: toDB(newStart), end_at: toDB(newEnd) })
      .eq('id', item.id);
    if (updateItemError) {
